...
public partial class sanjeevanAdmin_add_event : System.Web.UI.Page
{
    iClass c = new iClass();
    string rootPath = "";
            ...
            if (Request.QueryString["action"] != null)
            {
                if (Request.QueryString["id"] != null)
                {
                    if (Request.QueryString["action"] == "delete")
                    {
                        ...
                        ScriptManager.RegisterClientScriptBlock(this, GetType(), "CallMyFunction", "waitAndMove('" + Master.rootPath + "sanjeevanAdmin/add-event', 2000);", true);
   ...
   protected void btnSubmit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (fuImg.HasFile)
            {
                ...
                {
                    int maxId = c.NextId("Events", "npId");
                    string photoName = "notice-photo-" + maxId.ToString() + fExt;

                    //int galFlag = rdbGallery.Checked ? 2 : 1;
                    c.ExecuteQuery("Insert Into Events (npId, npImage, npImageTitle, galleryFlag) Values (" + maxId + ", '" + photoName + "', '" + txtTitle.Text + "', 1)");
